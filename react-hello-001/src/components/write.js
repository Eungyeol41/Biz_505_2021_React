 ...
 * 변수나 객체의 값이 바뀌면(변경되면) 그에 맞춰서 화면이 다시 rendering된다
 *
...
const Write = () => {
   ...
   * setB_Name() 함수에 문자열이 전달되면 내부에서는
   * 	1. 전달된 문자열을 임시 변수에 할당
   * 		const _b_name = 전달된문자열
   * 	2. 실제 b_name 변수가 가리키고 있는 메모리의 주소를 _b_name(임시변수) 주소로 변경한다
   * 	3. 결국 원래 b_name 변수는 그대로 유지되고 새로만들어진 _b_name 변수가 생성이 된다
   * 	4. react 원래 b_name 변수와 새로 생성된 _b_name 변수를 비교하여 변화가 있으면 rendering을 한다
   */
  const onWriterChangeHandle = (e) => {
    const b_writer_text = e.target.value;
    // react 에서는 상태(변수, 객체)에 직접 값을 할당해서는 절대 안 된다
